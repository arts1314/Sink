...
export const LinkSchema = z.object({
  ...
  url: z.string().trim().url().max(20048),
  slug: z.string().trim().max(20048).regex(new RegExp(slugRegex)).default(nanoid()),
  comment: z.string().trim().max(20048).optional(),
  ...
  title: z.string().trim().max(20048).optional(),
  description: z.string().trim().max(20048).optional(),
  image: z.string().trim().url().max(20048).optional(),
})
